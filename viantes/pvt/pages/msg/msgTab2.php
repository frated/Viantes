			...
			foreach ($msgDOArray as $key => $msgDO) { ?>
				<tr class="msgBodyRow <?php if ($i % 2 == 0)  { echo 'msgBodyRowBG'; } $i++; ?>">
					<td class="msgBodyCell">
						<a class="bodyColor" href="/viantes/pub/pages/profile/showMsg.php?msgId=<?php echo X_code($msgDO->getId()) ?>&tabactive=2">
					...
					<td class="msgBodyCell" onclick="javascript: window.location = '/viantes/pub/pages/profile/showMsg.php?msgId=<?php echo X_code($msgDO->getId()) ?>&tabactive=2'">
						<a class="bodyColor" href="/viantes/pub/pages/profile/showMsg.php?msgId=<?php echo X_code($msgDO->getId()) ?>&tabactive=2">
							<?php $lunghOriginale = strlen ( str_replace('<br />',' ', str_replace('<br>',' ', $msgDO->getMessage() )) ); 
								  $msg = str_replace('<br />',' ', str_replace('<br>',' ', substr($msgDO->getMessage(), 0, 30))); 
								  echo $msg;
								  if ( $lunghOriginale > 30 ) echo "..." ?>
						</a> 
					</td>
					<td class="msgBodyCell">
